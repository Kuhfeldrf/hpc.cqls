...
# Try pdb2gmx directly first - modern GROMACS handles HETATM fine
# The real issue is atom naming, not HETATM vs ATOM
echo -e "1\n1" | gmx pdb2gmx -f "${PDB_BASENAME}" -o processed.gro -water tip3p -ignh 2>&1 | tee pdb2gmx.log

# If it fails due to missing CA atoms, we'll need to fix the atom naming
if [ ${PIPESTATUS[1]} -ne 0 ]; then
    echo "pdb2gmx failed. Checking if atom naming needs to be fixed..."
    echo "This may require manual PDB file correction or using a different force field option."
    exit 1
fi
